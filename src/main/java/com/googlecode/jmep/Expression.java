...
public class Expression {
    ...
    private void tokenize() throws ExpressionException {
        ...
        this.tokenList.add(new Token(Type.MRK, 0));
        ...
        while (true) {
            ...
            switch (cc) {
                case StringCharacterIterator.DONE:
                    this.tokenList.add(new Token(Type.MRK, iterString.getIndex()));
                    return;
                case '(':
                    this.tokenList.add(new Token(Type.OPA, iterString.getIndex()));
                    cc = iterString.next();
                    continue;
                case ')':
                    this.tokenList.add(new Token(Type.CPA, iterString.getIndex()));
                    cc = iterString.next();
                    continue;
                case ',':
                    this.tokenList.add(new Token(Type.CMA, iterString.getIndex()));
                    cc = iterString.next();
                    continue;
            }
            ...
            if (Character.isUnicodeIdentifierStart(cc)) {
                ...
                if (lastToken.getType() == Type.VAL ||
                                lastToken.getType() == Type.VAR ||
                                lastToken.getType() == Type.UNI ||
                                lastToken.getType() == Type.CPA
                ) {
                ...
                } else if (lastToken.getType() != Type.UNI) {
            ...
            switch (cc) {
                ...
                case '<': {
                    ...
                    if (cc == '>') {
                        ...
                        cc = iterString.next();
                    } /* BUGFIX 9/30/2005 by Graham; Add 'else'. */ else if (cc == '=') {
